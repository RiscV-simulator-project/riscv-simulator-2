...
RiscV5StageVM::RiscV5StageVM() : VmBase() {
    // Constructor: Initialize any specific state for the 5-stage VM
    Reset();
}
...
void RiscV5StageVM::MemoryStage() {
    ...
    next_mem_wb_reg_.control = ex_mem_reg_.control; // Pass control signals
...
void RiscV5StageVM::ExecuteStage() {
    ...
    next_ex_mem_reg_.funct3 = id_ex_reg_.funct3; // Pass funct3 forward
    ...
    uint64_t alu_input1;
    // For JAL and AUIPC, the first ALU input is the PC. For others, it's rs1.
    if (id_ex_reg_.control.jump || (id_ex_reg_.control.alu_op == 0b11 && id_ex_reg_.rd_idx != 0)) { // JAL, JALR, AUIPC
    ...
    if (id_ex_reg_.control.jump) {
        branch_taken_ = true;
        // For JALR, the target is alu_result. For JAL, it's also alu_result (PC + imm).
        branch_target_ = next_ex_mem_reg_.alu_result;
        // For JALR, the LSB of the target address must be set to 0.
        if (((id_ex_reg_.instruction >> 12) & 0b111) == 0) { // Check if it's JALR (funct3=0)
            branch_target_ &= ~1ULL;
        }
    }
    // Handle conditional branches
        ...
        switch (id_ex_reg_.funct3) {
            case 0b000: take_branch = (next_ex_mem_reg_.alu_result == 0); break; // BEQ
            case 0b001: take_branch = (next_ex_mem_reg_.alu_result != 0); break; // BNE
            case 0b100: take_branch = (next_ex_mem_reg_.alu_result == 1); break; // BLT
            case 0b101: take_branch = (next_ex_mem_reg_.alu_result == 0); break; // BGE
            case 0b110: take_branch = (next_ex_mem_reg_.alu_result == 1); break; // BLTU
            case 0b111: take_branch = (next_ex_mem_reg_.alu_result == 0); break; // BGEU
        }
...
void RiscV5StageVM::Step() {
    ...
    std::cout << "Step for 5-stage pipeline is not yet implemented." << std::endl;
    ClockTick();
    DumpRegisters(globals::registers_dump_file_path, registers_);
    DumpState(globals::vm_state_dump_file_path);
}
